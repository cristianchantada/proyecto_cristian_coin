...
class ServerConectionError(exceptions.HTTPException):
    ...
    '''class EnterInDataBase():

    def __init__(self):
        self.con = sqlite3.connect(DATA_BASE)
        self.cur = self.con.cursor()

    def db_select_fetchall(self, msg):
        self.cur.execute(msg)
        all_rows = self.cur.fetchall()
        self.con.close()
        return all_rows

    def db_delete(self, msg):
        self.cur.execute(msg)
        self.con.commit()
        self.con.close()

    def db_insert_antihack(self, msg, secret_values_msg): 
        self.cur.execute(msg, secret_values_msg)
        self.con.commit()
        self.con.close() 
...
def calculate(coin_from, coin_to, quantity_from):
    if coin_from != "EUR":
        do_things_db = EnterInDataBase
        max_cripto = do_things_db.db_select_fetchall(f"SELECT sum(cantidad_to) FROM operations_table WHERE moneda_to = '{coin_from}'")
        max_cripto = max_cripto[0][0]
    ...
    do_things_db.db_delete("DELETE FROM stand_by_operation_table;")
    do_things_db.db_insert_antihack("INSERT INTO stand_by_operation_table (moneda_from, quantity_from, moneda_to, quantity_to, date, time, unitary_prize) VALUES (?,?,?,?,?,?,?)", (coin_from, quantity_from, coin_to, quantity_to, date, time, unitary_prize))
...
def secure_operation(dict_validated_operation):
    ...
    do_things_db = EnterInDataBase()
    saved_operation = do_things_db.db_select_fetchall("SELECT moneda_from, quantity_from, moneda_to, quantity_to, date, time, unitary_prize FROM stand_by_operation_table;")
    do_things_db.db_delete("DELETE FROM stand_by_operation_table;")
...
def select_all():
    ...
    do_things_db = EnterInDataBase()
    selection = do_things_db.db_select_fetchall("SELECT date, time, moneda_from, cantidad_from, moneda_to, cantidad_to, unitary_prize FROM operations_table;")
    return selection

def commit_operation(values_dict):

    do_things_db = EnterInDataBase()
    do_things_db.db_insert_antihack("INSERT INTO operations_table (date, time, moneda_from, cantidad_from, moneda_to, cantidad_to, unitary_prize) VALUES (?,?,?,?,?,?,?)", (values_dict["date"], values_dict["time"], values_dict["moneda_from"], values_dict["quantity_from"], values_dict["moneda_to"], values_dict["quantity_to"], values_dict["unitary_prize"]))

def my_wallet():

    do_things_db = EnterInDataBase()
    investment_eur = do_things_db.db_select_fetchall("SELECT sum(cantidad_from) FROM operations_table WHERE moneda_from = 'EUR'")
    recovered_eur = do_things_db.db_select_fetchall("SELECT sum(cantidad_to) FROM operations_table WHERE moneda_to = 'EUR'")
    ...
    for crypto in crypto_values:
        result_this_crypto_to = do_things_db.db_select_fetchall(f"SELECT sum(cantidad_to) FROM operations_table WHERE moneda_to = '{crypto}'")
        result_this_crypto_from = do_things_db.db_select_fetchall(f"SELECT sum(cantidad_from) FROM operations_table WHERE moneda_from = '{crypto}'")
